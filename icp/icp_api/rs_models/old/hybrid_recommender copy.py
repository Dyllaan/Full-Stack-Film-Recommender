...
from tensorflow.keras.layers import StringLookup, Embedding, Flatten, Dense, GlobalAveragePooling1D, TextVectorization 
...
class AdvancedCFRecommenderModel(tf.keras.Model):
    def __init__(self, user_dim, movie_dim, timestamp_dim, title_dim, genre_dim, embedding_dim=50, title_output_dim=32):
        super().__init__()
        ...
        # Timestamp embedding
        self.timestamp_embedding = Embedding(timestamp_dim, embedding_dim)

        # Movie title vectorization and embedding
        self.title_vectorization = TextVectorization(max_tokens=title_dim)
        self.title_embedding = Embedding(input_dim=title_dim, output_dim=title_output_dim)

        # Movie genres vectorization and embedding
        self.genre_vectorization = TextVectorization(max_tokens=genre_dim)
        self.genre_embedding = Embedding(input_dim=genre_dim, output_dim=embedding_dim)
    ...
    def call(self, inputs):
        ...
        timestamp_vector = self.timestamp_embedding(inputs["timestamp"])
        timestamp_vector = Flatten()(timestamp_vector)

        title_tokens = self.title_vectorization(inputs["title"])
        title_vector = self.title_embedding(title_tokens)
        title_vector = GlobalAveragePooling1D()(title_vector)

        genre_tokens = self.genre_vectorization(inputs["genres"])
        genre_vector = self.genre_embedding(genre_tokens)
        genre_vector = GlobalAveragePooling1D()(genre_vector)

        # Combine all features
        concat = tf.concat([user_vector, movie_vector, timestamp_vector, title_vector, genre_vector], axis=1)
